...
import { calculateCharacterEntropy } from "../lib/passwordExample";
...
export const InlineEntropyValue: React.FC<React.PropsWithChildren> = ({
  children,
}) => (
...
const EntropyDisplay: React.FC<EntropyDisplayProps> = ({
  ...
  const rawEntropy = calculateCharacterEntropy(passPhrase, charset.size);
  const wordEntropy =
    ...
    <>
      ...
      >
        ...
        <DialogContent>
          ...
          >
            <Typography variant="body2" color="text.secondary">
              ちなみに、「{maleNameBirthdayExample.password}」
              は
              <InlineEntropyValue>
                {" " + maleNameBirthdayExample.entropy.toFixed(1)} bits
              </InlineEntropyValue>
              （単語エントロピー）
            </Typography>
          </Box>
