...
`wordlist.csv` は人間や AI が直接編集してよいが、編集時点では読み仮名の重複が一時的に入っていてもよい。

ただし、公開用の `wordlist.csv` は `WordListSorter.py` を通し、同じ読み仮名の語は1件だけ残すこと。これはユーザーの利便性と、語彙数に基づくエントロピー表示の正確性を保つため。
